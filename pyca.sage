#
# pyca - Python Cellular Automata toolset (pronounced pizza)
#

# a cellular automaton rule is a tuple defined by three parameters
# (states_number, neighborhood_size, rule) or shortly (k, m, r)
class CA1D_rule () :
...
#    ca.Sf = [ [ list2int (list2bool (vector(int2list(i, ca.k, ca.k^(ca.m-1))) * ca.D[c]), ca.k) for c in xrange(ca.k) ] for i in xrange(2^(ca.k^(ca.m-1))) ]
#    ca.Sb = [ [ list2int (list2bool (ca.D[c] * vector(int2list(i, ca.k, ca.k^(ca.m-1)))), ca.k) for c in xrange(ca.k) ] for i in xrange(2^(ca.k^(ca.m-1))) ]
  ...
  def __repr__(ca):
    return "Cellular Automaton (states = "+str(ca.k)+", neighbors = "+str(ca.m)+", rule = "+str(ca.r)+")"
...
class CA1D_lattice () :
  ...
  # this code might be rewritten to extend the strings instead of using modulo operations
  # the boundary can be defined as [b_L, b_R], where the let ad right boundaries are lists
  def next (lt) :
    if (lt.b == 'cyclic') :
      lt.C = [ lt.ca.f[list2int([lt.C[(x+lt.ca.m-1-i-lt.sh) % lt.N] for i in xrange(lt.ca.m)], lt.ca.k)] for x in xrange(lt.N            ) ]
    else :
      lt.C = [ lt.ca.f[list2int([lt.C[ x+lt.ca.m-1-i              ] for i in xrange(lt.ca.m)], lt.ca.k)] for x in xrange(lt.N-(lt.ca.m-1)) ]
      lt.N = lt.N - (lt.ca.m-1)
    return
  ...
  def isGoE (lt) :
    if (lt.b == 'open') :
      ...
      return "Unsuported boundary"

# preimages are produced in the wrong order are not sorted correctly
